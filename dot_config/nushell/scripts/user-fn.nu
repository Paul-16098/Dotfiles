...
def app-update-job-list []: nothing -> table<name: string, fn: closure> {
  [
    ...
    [
      cargo-packages
      {
        cargo install-update --all --git --filter=!name=nu ...($in.exclude? | default [] | par-each { $"--filter=!name=($in)" }) o+e>| lines | each { print $in }
      }
    ]
    [
      nu-plugins
...
const APP_UPDATE_COMPLETE_EXCLUDE = []
...
export def app-update [
  ...
  for item in ($job_list | enumerate) {
    ...
    if $dry_run {
      print --stderr $"dry run: ($this_cofg) | (view source $item.item.fn)"
    } else {
      try {
        $this_cofg | do $item.item.fn
      } catch { print $in.rendered }
    }
  }
  pb clear
